...
if __name__ == "__main__":
    ...
    train_data_indices, test_data_indices = train_test_split(np.arange(len(OPENML_ID)),
                                                         test_size=0.2,
                                                         random_state=1)
    for trained_on in groups4.keys():
        for eval_on in groups4.keys():
            if trained_on == eval_on:
                continue
            print(f"{trained_on}, {eval_on}")
            for data_split_state in states:
                print(data_split_state)
                train_learner_indices = np.array(groups4[trained_on])
                test_learner_indices = np.array(groups4[eval_on])
